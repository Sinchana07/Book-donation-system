...
function getParticularauthor()
{
    ...
    if (isset($_GET['book'])) {
        $book_id = $_GET['book'];
        $fetch = "select * from `books` where book_id=$book_id";
                  $result = $con->query($fetch);
                  if ($result->num_rows > 0) {
                   while($row = $result->fetch_assoc()) {
        //$select_books = "select * from `books` whezre book_id = $book_id ";
        //$result_query = mysqli_query($con, $select_books);
       // $num_books = mysqli_num_rows($result_query);
        ////if ($num_books == 0) {
           // echo "<h2 class ='text-center text-danger'>No stock for books by this publisher</h2>";
        //}
        //while($row = mysqli_fetch_assoc($result_query)>0) {
            $book_id = $row['book_id'];
            $book_title = $row['book_title'];
            $description = $row['book_desc'];
            $book_author = $row['book_author'];
...
function getAuthors()
{
    ...
    while ($row_data = mysqli_fetch_assoc($result_books)) {
        ...
        echo "<li class='nav-item'>
                  <a href='index.php?book=$book_id' class='nav-link text-light'>$author_name</a>
                  </li>";
    }
}
...
function cart()
{
    if (isset($_GET['add_to_cart'])) {
        ...
        if ($num_of_rows > 0) {
            ...
            $insert_query = "insert into `cart_details` (book_id,ip_address,quantity) values ($get_book_id,'$get_ip_add',1)";
...
function cart_item()
{
    ...
    /*  function place_cart_item()
      {
        global $con;
        if (isset($_POST['place_cart'])) {
          foreach ($_POST['placeitem'] as $place_id) {
            $fetch = "select * from `book_delete` where book_id=$place_id";
            $result = $con->query($fetch);
            if ($result->num_rows > 0) {
              while ($row = $result->fetch_assoc()) {
            ...
            $insert_books_orders = "insert into `orders` (book_id,donor_ip,book_title,category_id,publisher_id,bImage1,date_added,status)
                        values ('$place_id','$ip_address','$fetch1', '$fetch5', '$fetch6', '$fetch7',NOW(), 'order success')";
            $insert_result = mysqli_query($con, $insert_books_orders);
            $delete_query = "Delete from `cart_details` where book_id=$place_id";
            $delete_trigger = "Delete from `book_delete` where book_id=$place_id";
            $run = mysqli_query($con, $delete_trigger);
            $run_delete = mysqli_query($con, $delete_query);
            if ($run_delete and $run) {
      ...
      echo $place_item = place_cart_item();*/
